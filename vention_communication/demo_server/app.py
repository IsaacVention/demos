...
from vention_state_machine.src.state_machine.defs import StateGroup, State, Trigger
from vention_state_machine.src.state_machine.decorators import (
    on_enter_state,
    on_exit_state,
    on_state_change,
    auto_timeout,
...
from vention_state_machine.src.state_machine.core import StateMachine, BaseStates, BaseTriggers
from demo_server.models import Quiz, Configuration, DEFAULT_MAX_BOX_HEIGHT, DEFAULT_TIMEOUT_SECONDS
...
class QuizMachine(StateMachine):
    ...
    def __init__(
        ...
        state_change_publisher: Optional[Callable[[str, str, str], None]] = None,
        *args,
        **kwargs,
    ):
        ...
        self.state_change_publisher = state_change_publisher
    ...
    def enter_fault(self, _):
        """Handle timeout by transitioning to fault state"""
        print("Entering fault state")
        
    @on_state_change
    def on_state_change(self, old_state: str, new_state: str, trigger: str):
        if self.state_change_publisher:
            print(f"Publishing state change: {old_state} → {new_state} (triggered by {trigger})")
            self.spawn(self.state_change_publisher(old_state, new_state, trigger))
